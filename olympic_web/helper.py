...
# country wise or year wise medal tally
...
def data_over_time(df,col):
    event_over_time = df.drop_duplicates(['Year', col])['Year'].value_counts().reset_index().sort_values('index')
    event_over_time.rename(columns={'index': 'Edition', 'Year': col}, inplace=True)
    return event_over_time


def mostsuccessful(df, sport):
    temp_df = df.dropna(subset=['Medal'])  # dropping those where madel is NAN
